...
for instruction in input:
    ...
    if "turn off" in instruction:
        grid[x_initial:x_end+1, y_initial:y_end+1] = 0

    elif "turn on" in instruction:
        grid[x_initial:x_end+1, y_initial:y_end+1] = 1

    else:
        grid[x_initial:x_end+1, y_initial:y_end+1] = np.where((grid[x_initial:x_end+1, y_initial:y_end+1]==0)|(grid[x_initial:x_end+1, y_initial:y_end+1]==1),
                                                               grid[x_initial:x_end+1, y_initial:y_end+1]^1,
                                                               grid[x_initial:x_end+1, y_initial:y_end+1])
